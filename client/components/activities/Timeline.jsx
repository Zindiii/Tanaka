...
function getPremiumForClient(name, organizations, leads, activitiesForClient) {
  if (Array.isArray(activitiesForClient) && activitiesForClient.some((a) => !!a.premiumSupport)) return true;
...
export default function Timeline({ activities, organizations }) {
  ...
  const clients = React.useMemo(() => Array.from(byClient.keys()).sort(), [byClient]);

  return (
    <Card className="border-blue-200">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">Client Activity Timeline</CardTitle>
          <Badge variant="outline" className="bg-blue-50 text-blue-800 border-blue-200">{clients.length} Active Clients</Badge>
        </div>
        <CardDescription>Visual timeline showing activity progression for each client</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {clients.map((client) => {
          ...
          const isPremium = getPremiumForClient(client, organizations, leads, items);
          const initial = client?.trim()?.charAt(0)?.toUpperCase() || "?";

          return (
            <div key={client} className="rounded-lg border bg-white/90 backdrop-blur-sm p-4 hover:shadow-sm transition-shadow">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {isPremium && (
                        <Badge variant="outline" className="bg-yellow-100 text-yellow-800 border-yellow-200 inline-flex items-center gap-1">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                      ...
                      {lastDays != null && <span>• last {lastDays === 0 ? "today" : `${lastDays}d ago`}</span>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pl-3">
                <div className="relative">
                  <div className="absolute left-3 top-0 bottom-0 w-px bg-blue-100" aria-hidden />
                        ...
                        <div key={a.id || idx} className="relative flex gap-4">
                          ...
                          <div className="ml-6 flex-1">
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              {a.status && (
                                <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200">{a.status}</Badge>
                              )}
                              {a.ticketType && (
                                <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">{a.ticketType}</Badge>
                              )}
                              {a.premiumSupport && (
                                <Badge variant="outline" className="bg-yellow-100 text-yellow-800 border-yellow-200 flex items-center gap-1"><Star className="h-3 w-3" />Premium</Badge>
                              )}
                            </div>
                            {a.notes && (
                              <p className="text-sm mt-1 text-foreground/90">{a.notes}</p>
                            )}
                            {Array.isArray(a.responsible) && a.responsible.length > 0 && (
                              <p className="text-xs text-muted-foreground mt-1">Responsible: {a.responsible.join(", ")}</p>
                            )}
                          </div>
                        </div>
                      );
